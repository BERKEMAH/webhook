...
import (
	...
	"github.com/BERKEMAH/webhook"
	"github.com/BERKEMAH/webhook/functions"
...
func PostBalasan(w http.ResponseWriter, r *http.Request) {
	...
	if r.Header.Get("Secret") == os.Getenv("SECRET") {
		if msg.Message == "loc" || msg.Message == "Loc" || msg.Message == "lokasi" || msg.LiveLoc {
			...
			res, _ := atapi.PostStructWithToken[atmessage.Response]("Token", os.Getenv("TOKEN"), dt, "https://www.berkemah.my.id/login")
			dat := &wa.TextMessage{
				To:       msg.Phone_number,
				IsGroup:  false,
